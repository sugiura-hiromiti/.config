...
async fn get_applescript_focused_window() -> Result<String> {
    let output = Command::new("osascript")
        .args(["-e", "tell application \"System Events\" to get name of window 1 of (first application process whose frontmost is true)"])
        .output()?;
    ...
    let window_title = String::from_utf8(output.stdout)?
        .trim()
        .to_string();
