use crate::helpers::colors::{self, Colors};
use crate::helpers::icons::{self, Icons};
...
pub async fn update(bar: &SketchyBar) -> Result<()> {
    ...
    debug!("🔋 Battery updated: {}% ({})", label, if batt_info.contains("AC Power") { "charging" } else { "discharging" });
    Ok(())
}
