return {
	...
	config = function()
		...
		local function hl()
			return vim.o.background == 'light' and require('catppuccin.palettes').get_palette 'mocha'
		...
		--local mode = { init = function(self) self.mode = vim.fn.mode() end, static = { mode_names = { n = 'N', no = 'N?', nov = 'N?', noV = 'N?', ['no\22'] = 'N?', niI = 'Ni', niR = 'Nr', niV = 'Nv', nt = 'Nt', v = 'V', vs = 'Vs', V = 'V_', Vs = 'V_s', ['\22'] = 'V|', ['\22s'] = 'V|s', s = 'S', S = 'S_', ['\19'] = 'S|', i = 'I', ic = 'Ic', ix = 'Ix', R = 'R', Rc = 'Rc', Rx = 'Rx', Rv = 'Rv', Rvc = 'Rvc', Rvx = 'Rvx', c = 'C', cv = 'Ex', r = '…', rm = 'M', ['r?'] = '?', ['!'] = '!', t = 'T', }, mode_hls = function() return { n = hl().blue, i = hl().green, v = hl().mauve, V = hl().mauve, ['\22'] = hl().mauve, c = hl().yellow, s = hl().lavender, S = hl().lavender, ['\19'] = hl().lavender, r = hl().sky, R = hl().sky, t = hl().red, ['!'] = hl().red, } end, }, provider = function(self) return '%2(' .. self.mode_names[self.mode] .. '%) ' end, hl = function(self) local mode = self.mode:sub(1, 1) return { fg = hl().text, bg = self.mode_hls()[mode], bold = true } end, update = 'ModeChanged', callback = vim.schedule_wrap(function() vim.cmd 'redrawstatus' end), }
		...
		local icon = {
			init = function(self)
				self.icon, self.icon_color = require('nvim-web-devicons').get_icon_colors_by_filetype(vim.bo.filetype)
			end,
		...
		local diags = {
...

			{
				...
				provider = function(self)
					return symbols.DiagnosticError .. ' ' .. self.errors .. ' '
				end,
			...
			},
			{
				...
				provider = function(self)
					return symbols.DiagnosticWarn .. ' ' .. self.warns .. ' '
				end,
			...
			},
			{
				...
				provider = function(self)
					return symbols.DiagnosticInfo .. ' ' .. self.infos .. ' '
				end,
			...
			},
			{
				...
				provider = function(self)
					return symbols.DiagnosticHint .. ' ' .. self.hints .. ' '
				end,
		...
		local git = {
			...
			},
			{
				...
				hl = { fg = hl().red, bg = hl().base },
			},
			{
				provider = function(self)
					return '  ' .. self.status_dict.head .. ' '
				end,
				hl = { fg = hl().base, bg = hl().peach, bold = true },
			},
		}

		local statusline = {
			{ ruler, path_block, navic, align, diags, align, git, lsp_attached },
		}
