...
var require_remove = __commonJS((exports2, module2) => {
  ...
  function remove2(path5, callback) {
    if (fs3.rm)
      return fs3.rm(path5, {recursive: true, force: true}, callback);
    rimraf(path5, callback);
  }
  function removeSync(path5) {
    if (fs3.rmSync)
      return fs3.rmSync(path5, {recursive: true, force: true});
    rimraf.sync(path5);
  }
  module2.exports = {
    remove: u(remove2),
...
function version(ctx) {
  return async () => {
    ...
    import_coc2.window.showMessage(v);
  };
}
...
var Config = class {
  ...
  get inlayHints() {
    ...
    return {
      ...
      typeHintsPrefix: cfg.get("inlayHints.typeHintsPrefix", "\xAB "),
      paramHintsPrefix: cfg.get("inlayHints.paramHintsPrefix", "\xBB "),
      trimSemicolon: cfg.get("inlayHints.trimSemicolon", true),
      refreshOnInsertMode: hasVirtualText && cfg.get("inlayHints.refreshOnInsertMode", false)
    };
  }
};
...
var InlayHintsController = class {
  constructor(ctx) {
    ...
    this.nvim = import_coc4.workspace.nvim;
    this.nvim.createNamespace("nvim_create_namespace").then((id) => {
      this.config.ns = id;
    });
    this.nvim6 = import_coc4.workspace.has("nvim-0.6.0");
    import_coc4.workspace.onDidChangeConfiguration(this.setConfiguration, this, this.disposables);
  }
  setConfiguration(e) {
    if (e && !(e.affectsConfiguration("sumneko-lua") || e.affectsConfiguration("Lua")))
      return;
...
var Ctx2 = class {
  ...
  resolveBin() {
    const serverDir = this.config.serverDir ? this.config.serverDir : import_path3.default.join(this.extCtx.storagePath, "sumneko-lua-ls", "extension", "server");
  ...
  async getCurrentVersion() {
    if (this.config.serverDir) {
      ...
      args.push("--version");
      try {
        return String((0, import_child_process.execSync)(`${cmd} ${args.join(" ")}`)).trim();
      } catch (err) {
        console.log(err);
        return;
      }
...
async function activate(context) {
  ...
  if (!bin) {
    ...
    if (ret == "Yes") {
      ...
      import_coc6.window.showMessage(`You can run ':CocCommand sumneko-lua.install' to install server manually or provide setting 'serverDir'`);
      return;
    }
  }
  ...
  if (ctx.config.checkUpdate) {
    const dataPath = import_path4.default.join(context.storagePath, "checkUpdate");
    let lastCheck = 0;
